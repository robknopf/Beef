...
\						NOP!();
			...
			else if ((gApp.GetBuildToolset(workspaceOptions) == .GNU) || (useLinuxLLVM))
			{
				...
				if ((options.mBuildOptions.mBuildKind == .DynamicLib) && (!useLinuxLLVM) && (mPlatformType == .Linux))
				{
				...
				if ((options.mBuildOptions.mBuildKind == .StaticLib) ||
					((options.mBuildOptions.mBuildKind == .DynamicLib) && (!useLinuxLLVM) && (!isStandaloneDynLib)))
				{
